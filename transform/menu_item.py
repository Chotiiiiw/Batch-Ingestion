...
# 1. Read MongoDB-shaped JSONL and restaurant source data.
...
# 2. Clean fields and support both current and legacy price names.
current_price = (
    col("basePrice")
...
cleaned_menu_items = (
    menu_items
    .withColumn("menu_item_id", trim(col("_id")))
    .withColumn("restaurant_id", col("restaurantId").cast("long"))
    .withColumn("menu_item_name", trim(col("name")))
    ...
    .withColumn("available", col("available").cast("boolean"))
    .withColumn(
        "tags",
        array_sort(
            coalesce(
...
restaurant_lookup = (
    restaurants
    .select(
        col("restaurant_id").cast("long").alias("known_restaurant_id")
    )
...
checked_menu_items = joined_menu_items.withColumn(
    ...
    .when(
        col("base_price").isNull() | (col("base_price") < 0),
...
rejected_menu_items = checked_menu_items.filter(
...
# 6. Hash the cleaned business attributes for SCD Type 2.
