...
# 2. Clean, cast, and recalculate the authoritative order total.
...
cleaned_orders = orders\
    ...
    .withColumn("delivery_fee",col("delivery_fee").cast("decimal(12,2)"))\
    .withColumn("source_total_amount",col("total_amount").cast("decimal(12,2)"))\
    .withColumn("total_amount",col("subtotal") - col("discount") + col("delivery_fee"))\
...
checked_orders = cleaned_orders.withColumn(
    "rejection_reason",
    when(
        col("order_id").isNull() | (col("order_id") <= 0),
    ...
    .when(
        col("subtotal").isNull() | (col("subtotal") < 0),
    ...
    .when(
        col("delivery_fee").isNull() | (col("delivery_fee") < 0),
...
rejected_orders = checked_orders.filter(
...
valid_orders = (
    ...
    .withColumn(
        "total_was_corrected",
        col("source_total_amount") != col("total_amount"),
    )
)


# 5. Prepare warehouse-ready natural-key columns.
...
print(
    "Corrected totals:",
    valid_orders.filter(col("total_was_corrected")).count(),
)
