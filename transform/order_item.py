...
if __package__:
    from transform.common import bangkok_date_key, create_spark, read_mongo_collection, read_mysql_table
else:
    from common import bangkok_date_key, create_spark, read_mongo_collection, read_mysql_table
...
def transform_order_items(
    ...
    # 1. Clean and cast order-item columns
    cleaned_order_items = order_items \
        ...
        .withColumn(
            "menu_item_id",
            trim(col("menu_item_id")),
        ) \
        .withColumn(
            "menu_item_name",
            trim(col("menu_item_name")),
        ) \
        .withColumn(
            "quantity",
    ...
    # 2. Prepare order and menu lookups
    ...
    menu_lookup = menu_items \
        .select(
            trim(col("_id"))
            .alias("lookup_menu_item_id"),
            col("restaurantId")
            .cast("long")
            .alias("menu_restaurant_id"),
        ) \
        .dropDuplicates(["lookup_menu_item_id"])


    # 3. Join source lookups
    joined_order_items = cleaned_order_items \
        .join(
            order_lookup,
            cleaned_order_items.order_id
    ...
    rejected_order_items = checked_order_items \
        .filter(col("rejection_reason").isNotNull())

    valid_order_items = checked_order_items \
    ...
    return warehouse_order_items, rejected_order_items
...
def main():
    ...
    # 1. Read order items and orders from MySQL
    ...
    # 2. Read menu items from MongoDB
