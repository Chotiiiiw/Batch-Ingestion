...
from common import bangkok_date_key, create_spark, optional_text

#CreateSparkSession
spark = create_spark("transform-payments")


# 1. Read payment and parent-order source data.
payments = spark.read\
    .option("header", True)\
    .csv("mock-data/output/payments.csv")
...
cleaned_payments = payments\
    ...
    .withColumn("transaction_ref", optional_text("transaction_ref"))\
    .withColumn("payment_created_at",to_timestamp(col("created_at")))\
    .withColumn("paid_at", to_timestamp(col("paid_at")))\
    .withColumn("source_updated_at",to_timestamp(col("updated_at")))\


# 3. Join to orders for customer, restaurant, and reconciliation amount.
...
order_lookup = orders.select(
    ...
    col("subtotal").cast("decimal(12,2)").alias("order_subtotal"),
    col("discount").cast("decimal(12,2)").alias("order_discount"),
    col("delivery_fee").cast("decimal(12,2)").alias("order_delivery_fee"),
)
...
joined_payments = cleaned_payments.join(
    order_lookup,
    cleaned_payments.order_id == order_lookup.lookup_order_id,
    "left",
)
...
checked_payments = joined_payments.withColumn(
    "rejection_reason",
    when(
        col("payment_id").isNull() | (col("payment_id") <= 0),
    ...
    .when(
        col("amount") != col("expected_payment_amount"),
    ...
    .when(
        col("source_updated_at") < col("payment_created_at"),
    ...
    .when(
        col("payment_status").isin("PAID", "REFUNDED")
        & col("paid_at").isNull(),
        lit("MISSING_PAID_AT"),
    )
    .when(
        col("paid_at").isNotNull()
        & (col("paid_at") < col("payment_created_at")),
...
rejected_payments = checked_payments.filter(
